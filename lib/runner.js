...
const asNumber = (v) => Number.parseInt(v, 10);
...
const readFuctionSource = (file, startLine, endLine, fnLength, errorLine) => {
        ...
        s.tests
          ...
          .forEach((test) => {
            ...
            if (ok) {
              ...
              const errorLineNumber = getLineNumber(filename, stack, Number.parseInt(lineNumber, fnLength) + 10);
              report.failed.push({
                title: `${x} ${s.title} -> ${test.title}`,
                path,
                code: readFuctionSource(
                  filename, lineNumber, errorLineNumber + 5, fnLength, errorLineNumber
                ) + '\n',
